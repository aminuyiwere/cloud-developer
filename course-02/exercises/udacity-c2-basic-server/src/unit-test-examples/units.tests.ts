...
describe('concat', ()=>{
  it('should concat "my" and "day"',()=>{
    const result = concat('my','day');
    expect(result).to.equal("myday");
  });
  it('throw an error if string is empty',()=>{
    expect(()=>{concat('','amin')|| concat('amin','')}).to.throw('Empty string');
  })
});
